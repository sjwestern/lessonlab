"""Command-line entrypoint for the lesson runtime."""
    ...
    for port in range(preferred_port, preferred_port + 20):
        try:
            return http.server.ThreadingHTTPServer(
                (bind_address, port), handler_class
            )
        except OSError:
            continue
    raise OSError(
        f"No available port from {preferred_port} to {preferred_port + 19}."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if problems:
        for problem in problems:
            print(f"lesson-server: {problem}", file=sys.stderr)
        return 1

    try:
        lesson_path = resolve_lesson(index, args.lesson)
    except ValueError as error:
        print(f"lesson-server: {error}", file=sys.stderr)
        return 1
